...
public:
    ...
    void replaceWord(string a, string b);
...
linkedList::linkedList() // constructor of our linked list
{
    ...
    { // while loop for running the operations continuously until exited
        ...
        else if (num == 12) // exit editor
        {
            ...
            cout << "Replace which word?" << endl;
            string str; cin >> str;
            cout << "Replace with what word?" << endl;
            string str1; cin >> str1;
            replaceWord(str,str1);
        }
        else // error message
        {
...
template <typename T>
vector<T> Split(const std::string& s, const string& seps = " ") {
	vector<T> ret; 
    T val;
...
void linkedList::replaceWord(string a, string b){
    Node *temp = head;
    bool flag = false;
    while(temp){
        string myLine = temp->data;
        vector<string> v = Split<string>(myLine);
